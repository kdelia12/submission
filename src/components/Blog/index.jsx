...
const ThumbnailTag = tw.span`absolute right-4 top-4 inline-block rounded-lg bg-blue-500 px-4 py-2 text-amber-500`;

const BlogContent = tw.div`p-4 dark:bg-[#1D2144] h-full`;

const Divider = tw.div`h-[0.5px] bg-black mb-8`;

const AuthorContentWrapper = tw.div`flex gap-4 text-black dark:text-amber-500`;
...
const Blog = () => {
  return (
    <section className="blog section-spacing dark:bg-black">
      <Container>
                ...
                <BlogContent>
                  <Link
                    className={`mb-4 inline-block text-[1.2rem] font-bold 
                               ...
                               dark:text-amber-500 dark:hover:text-blue-500`}
                    to="/"
                  >
                    {blog.title}
                  </Link>
